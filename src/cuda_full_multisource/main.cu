/**
 * Full multisource Enzyme + CUDA example
 *
 * In this example, we allow device kernels to call kernels across translation units
 * (CUDA multisource) and we call Enzyme autodiff on functions that call functions in
 * other TUs (Enzyme multisource).
 *
